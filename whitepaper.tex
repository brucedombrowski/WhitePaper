...
REQ-3.1 & \texttt{RandomNumberGenerator} usage & Inspection \\
\bottomrule
\end{tabularx}
\end{table}
...
The project's compliance scope spans six federal standards and publications:

\begin{enumerate}[leftmargin=*]
    \item \textbf{FIPS 197}: AES algorithm specification
    \item \textbf{FIPS 140-2}: Cryptographic module validation
    \item \textbf{NIST SP 800-132}: Password-based key derivation
    \item \textbf{NIST SP 800-38A}: Block cipher modes of operation
    \item \textbf{NIST SP 800-90A}: Random number generation
    \item \textbf{32 CFR Part 2002}: CUI marking and handling
\end{enumerate}
...
A JSON-formatted requirements specification containing 28 requirements across six categories: encryption algorithm, key derivation, random number generation, password handling, file format, and platform requirements. The JSON source-of-truth enables automated generation of formatted PDF documents via a Python build script.
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{DM-003}: Password transmission method (out-of-band per NIST SP 800-63B)
...
A line-by-line code verification mapping all 28 requirements to specific implementation evidence in the source code, including file paths, function names, and configuration values.
...
\begin{equation}
\text{Output} = \text{Salt}_{128} \| \text{IV}_{128} \| \text{AES-256-CBC}(K, \text{IV}, \text{Plaintext})
\end{equation}
...
The implementation uses exclusively platform-provided cryptographic libraries (\texttt{System.Security.Cryptography}), avoiding third-party dependencies that would complicate FIPS validation. When Windows FIPS mode is enabled, the tool leverages CMVP Certificate \#4515 for validated cryptographic operations.
...
\begin{lstlisting}[style=powershell, caption={Decryption one-liner (simplified)}, label={lst:oneliner}]
$f=Read-Host "File"
...
This design requires no software installation by recipients---only PowerShell, which is built into every modern Windows installation. The AI agent helped iterate on this one-liner to minimize its length while maintaining readability and compliance with the cryptographic parameter requirements.
...
Agent definitions are stored in a JSON configuration file that specifies each agent's role, permissions, and persistent context. Listing~\ref{lst:agent-config} shows the project setup agent configuration used in this paper's development.

\begin{lstlisting}[style=json, caption={Agent configuration excerpt (agents.json)}, label={lst:agent-config}]
{
  "name": "project-setup",
  "description": "Initialize compliance project
    structure and documentation templates",
  "instructions": "You are a project setup agent
...
\begin{enumerate}[leftmargin=*]
    \item \textbf{Model knowledge currency}: LLM training data has a cutoff date, meaning recent revisions to standards (e.g., updates to NIST SP 800-171 Rev.~3) may not be reflected. Developers must verify that AI-cited standards are current.
